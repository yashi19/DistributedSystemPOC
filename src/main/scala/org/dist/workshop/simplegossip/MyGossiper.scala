...
import org.dist.workshop.simplegossip.digest.{MyGossipDigest, MyGossipDigestSyn}

import scala.jdk.CollectionConverters._

class MyGossiper(val seed: InetAddressAndPort,
  ...
  var messagingService: MyMessagingService = _

  def setMessageService(messagingService: MyMessagingService): Unit = {
    this.messagingService = messagingService
  }

  def makeGossipDigestAck2Message(deltaEpStateMap: util.HashMap[InetAddressAndPort, BigInteger]) = {
    val gossipDigestAck2 = MyGossipDigestAck2(deltaEpStateMap.asScala.toMap)
  ...
  def makeGossipDigestAckMessage(deltaGossipDigest: util.ArrayList[GossipDigest], deltaEndPointStates: util.HashMap[InetAddressAndPort, BigInteger]) = {
    val digestAck = MyGossipDigestAck(deltaGossipDigest.asScala.toList, deltaEndPointStates.asScala.toMap)
  ...
  def start() = {
    executor.scheduleAtFixedRate(new MyGossipTask, intervalMillis, intervalMillis, TimeUnit.MILLISECONDS)
  }


  class MyGossipTask extends Runnable {
    ...
    def makeGossipDigestSynMessage(gDigests: util.List[MyGossipDigest]) = {
      val gDigestMessage = new MyGossipDigestSyn("TestCluster", gDigests)
    ...
    def makeRandomGossipDigest() = {
      ...
      val digests = new util.HashSet[MyGossipDigest]()
      /* Add the local endpoint state */
      var epState = endpointStatemap.get(localEndPoint)
      val localDigest = MyGossipDigest(localEndPoint, 1, 1)
      ...
      for (liveEndPoint <- endpoints.asScala) {
        ...
        if (epState != null) {
          digests.add(MyGossipDigest(liveEndPoint, 1, 1))
        }
        else digests.add(MyGossipDigest(liveEndPoint, 0, 0))
      }
